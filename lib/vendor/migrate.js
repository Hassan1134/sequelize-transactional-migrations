...
// Minimal deep-diff shape used by the adapted upstream action parser.
const diff = (left, right) => {
  const differences = [];
  const walk = (lhs, rhs, path = []) => {
    if (Object.is(lhs, rhs)) return;
    if (Array.isArray(lhs) || Array.isArray(rhs)) {
      if (Array.isArray(lhs) && Array.isArray(rhs) &&
        stableStringify(lhs) === stableStringify(rhs)) return;
      differences.push({ kind: "A", path, lhs, rhs });
      return;
    }
    const lhsObject = lhs && typeof lhs === "object";
    const rhsObject = rhs && typeof rhs === "object";
    if (!lhsObject || !rhsObject) {
      differences.push({ kind: "E", path, lhs, rhs });
      return;
    }
    for (const key of new Set([...Object.keys(lhs), ...Object.keys(rhs)])) {
      if (!Object.hasOwn(rhs, key)) differences.push({ kind: "D", path: [...path, key], lhs: lhs[key] });
      else if (!Object.hasOwn(lhs, key)) differences.push({ kind: "N", path: [...path, key], rhs: rhs[key] });
      else walk(lhs[key], rhs[key], [...path, key]);
    }
  };
  walk(left, right);
  return differences;
      ...
      case 'N':
        {
          ...
          if (df.path.length === 1) {
            ...
            Object.values(df.rhs.schema).forEach((v) => { if (v.references) depends.push(v.references.model) });
...
module.exports = { getMigration, sortActions, parseDifference, reverseModels };
